...
async function handle(text, actor) {
  ...
  if (stack.enabled()) {
    try {
      ...
      const diag = await stack.status().catch(() => null);
      return respond('The Agent Stack didn’t answer.', 'Error: ' + e.message, (diag && diag.hint) || 'Try again in about 60 seconds; free-tier services sleep when idle.', 'Check the Stack light in the status line below the console.', diag);
    }
  }
